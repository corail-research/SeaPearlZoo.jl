...
function solve_kidneyexchange_matrix(inputData::InputData)
    ...
    num_compatibilities = length(compatibilities)
    
    # =========VARIABLES=========
    #x[i, j] = 1 => pair i receive a kidney from pair j
    #x[i, j] = 0 => pair i do not receive a kidney from pair j
    x = Matrix{SeaPearl.AbstractIntVar}(undef, num_compatibilities, num_compatibilities)

    #minus_x = x * -1
    #Usefull to check with SumToZero that for each pair: give a kidney <=> receive a kidney
    minus_x = Matrix{SeaPearl.AbstractIntVar}(undef, num_compatibilities, num_compatibilities) 

    for i = 1:num_compatibilities
        for j = 1:num_compatibilities
    ...
    # =========OBJECTIVE=========
    minusNumberOfExchanges = SeaPearl.IntVar(-num_compatibilities, 0, "minusNumberOfExchanges", trailer) # SeaPearl minimizes the objective, so we min the negative objective 
    SeaPearl.addVariable!(model, minusNumberOfExchanges; branchable=false)
    vars = SeaPearl.AbstractIntVar[]

    for i in 1:num_compatibilities
