...
###############################################################################
######### Compare heterogeneous with standard representation
#########  
######### 
###############################################################################

function simple_experiment_htg_std_GC(n_nodes, k, n_episodes, n_instances; chosen_features=nothing, feature_size=nothing, n_eval=25, n_eva = n_nodes, k_eva = k,n_layers_graph=3, reward = SeaPearl.GeneralReward, c=2.0,  trajectory_capacity = 20000, pool = SeaPearl.meanPooling(), nbRandomHeuristics = 1, eval_timeout = 240, restartPerInstances = 1, seedEval = nothing, device = gpu, batch_size = 64, update_freq = 1,  target_update_freq= 100, name = "", numDevice = 0, eval_strategy = SeaPearl.DFSearch())
        """
    Runs a single experiment on GC
    """
    n_step_per_episode = n_nodes
    ...
    density = 0.9
    generator = SeaPearl.ClusterizedGraphColoringGenerator(n_nodes, n_min_color, density)
    eval_generator = SeaPearl.ClusterizedGraphColoringGenerator(n_eva, n_min_color, density)

    evalFreq=Int(floor(n_episodes / n_eval))

    step_explorer = Int(floor(n_episodes*n_step_per_episode*0.5 ))
    ...
    get_default_trajectory = () -> get_default_slart_trajectory(capacity= trajectory_capacity, n_actions= n_nodes),   
    ...
    get_default_nn = () -> get_default_cpnn(
        feature_size= 14,       #depends on the chosen_features
        conv_size=8,
        dense_size=16,
        output_size=n_nodes,
        n_layers_graph=3,
    ...
    agent_heterogeneous_chosen = get_heterogeneous_agent(;
    get_heterogeneous_trajectory = () -> get_heterogeneous_slart_trajectory(capacity=trajectory_capacity, n_actions=n_nodes),        
    get_explorer = () -> get_epsilon_greedy_explorer(step_explorer, 0.1; rng = rngExp ),
    batch_size=batch_size,
    update_horizon=update_horizon,
    ...
    learned_heuristic_heterogeneous_chosen = SeaPearl.SimpleLearnedHeuristic{SR_heterogeneous,reward,SeaPearl.FixedOutput}(agent_heterogeneous_chosen; chosen_features=chosen_features)

    learnedHeuristics = OrderedDict(
        "homogeneous" => learned_heuristic_default_chosen,
        "heterogeneous" => learned_heuristic_heterogeneous_chosen,
    )
    ...
    metricsArray, eval_metricsArray = trytrain(
        ...
        seedEval=seedEval,
        nbRandomHeuristics=nbRandomHeuristics,
        exp_name=name *'_'* string(n_nodes) *"_"*string(n_eva) * "_" * string(n_episodes) * "_"* string(seedEval) * "_",
        eval_timeout=eval_timeout,
