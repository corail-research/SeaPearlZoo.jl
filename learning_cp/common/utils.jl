...
function get_default_cpnn(;feature_size, conv_size, dense_size, output_size, n_layers_graph, n_layers_node, n_layers_output, pool=SeaPearl.sumPooling(), σ=Flux.leakyrelu, init = Flux.glorot_uniform, device = cpu)
...
function get_heterogeneous_cpnn(;feature_size, conv_size=8, dense_size=16, output_size, n_layers_graph=3, n_layers_node=2, n_layers_output=2, pool=SeaPearl.meanPooling(), σ=Flux.leakyrelu, init = Flux.glorot_uniform, device = cpu)
    return SeaPearl.HeterogeneousCPNN(
        ...
        nodeChain=get_dense_chain(conv_size, dense_size, dense_size, n_layers_node, σ, init = init),
        outputChain=get_dense_chain(dense_size, dense_size, output_size, n_layers_output, σ, init = init)|> device
    )
end
