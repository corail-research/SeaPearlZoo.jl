...
function build_graph_coloring_actor_model(output_size:: Int)
...
function build_graph_coloring_critic_model(output_size:: Int)
    approximator_model = SeaPearl.CPNN(
        graphChain = Flux.Chain(
            SeaPearl.GraphConv(numInFeatures => 16, Flux.leakyrelu),
            SeaPearl.GraphConv(16 => 16, Flux.leakyrelu),
...
function build_graph_coloring_ppo_agent(actor_model, critic_model, agent_config::ColoringPPOAgentConfig)
    agent = RL.Agent(
        policy = RL.PPOPolicy(
